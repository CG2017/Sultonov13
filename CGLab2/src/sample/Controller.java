...
public class Controller {
    ...
    void transformImage(ActionEvent event) throws InterruptedException {
        ...
        try {
            ...
            /*if( faultTextField.getText().trim().length()== 0 || faultTextField.getText() == null)
            {
                fault = (int) faultSlider.getValue();


            }
            else {
                fault = Integer.parseInt(faultTextField.getText());
                faultSlider.setValue(Double.parseDouble(faultTextField.getText()));

            }*/
            fault = (int) faultSlider.getValue();
            faultTextField.setText(String.valueOf(fault));
            ...
            protected Void call() throws Exception {
                for(int x = 0; x < width; x++) {
                    for(int y = 0; y < height; y++) {
                        ...
                        ColorCIELab colorBefore = ColorConversions.convertXYZtoCIELab(ColorConversions.convertRGBtoXYZ(colorRGBBefore.getRGB()));
    ...
    private void setSliderListeners() {
        ...
        /*ChangeListener<Number> changeSlider = (observable, oldValue, newValue)->{
            faultSlider=faultSlider.getValue();
            faultTextField.setText(faultSlider);
        };*/

        //faultSlider.valueProperty().addListener(changeSlider);
    ...
    void initialize() {
        try {
            ...
            faultSlider.setMajorTickUnit(5);
            faultSlider.setMajorTickUnit(5);
            faultSlider.setShowTickMarks(true);
